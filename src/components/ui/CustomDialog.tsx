...
export default function CreateChatDialog({ isOpen, handleClose, children }: { isOpen: boolean, handleClose: () => void, children: React.ReactNode }) {
    return (
        <>
            <div onClick={handleClose} className={"absolute left-0 top-0 w-screen h-screen flex items-center justify-center bg-black/50 z-50 transition-all duration-200" + (isOpen ? " opacity-100 pointer-events-auto " : " opacity-0 pointer-events-none ")}>
